...
def create_canton_map_visualization(job_counts_input_path: Path, report_output_path: Path, job_per_canton_output_path: Path):
    try:
        ...
        df_sum = df_job_count.groupby("canton", as_index=False)["job_count"].sum()
        # Check for duplicates in cantons
        print("=== df_sum  ===")
        print(df_sum)
        # Sort descending
        df_per_canton = df_sum.sort_values("job_count", ascending=False)
        ...
        # ----------------------------------------------------------
        # Merge the gdf with the df_per_canton & save as csv
        # ----------------------------------------------------------

        # merge the gdf wit df_per_canton
        ...
        # ----------------------------------------------------------
        # Define the color bins, labels, and colors
        # ----------------------------------------------------------

        # The color scheme was chosen according to color brewer, and on purpose there is a bin with no corresponidng
        # values to visually indicate the big gap between ZH and other cantons.
        ...
        # Fix invalid geometries before computing centroids (exclaves)
        ...
        # Manual fine-tuning for small or overlapping cantons
        label_offsets = {
            "BS": (3000, 6000),
            "BL": (10000, -2000),
            "GE": (3800, 1200),
